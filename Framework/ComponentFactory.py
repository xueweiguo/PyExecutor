...
class ComponentFactory(object):
    ...
    def make_element(self, cd, type):
        if type == "Custom":
            return CustomBlock(cd, 'Custom').construct(None)
        elif type == "Input":
            return InputBlock(cd, 'Input', '用户取得自定义功能的输入数据').construct(None)
        elif type == "Output":
            return OutputBlock(cd, 'Output','用于设定用户自定义模块的输出数据').construct(None)
        elif type == "Note":
            return Note(cd).construct(None)
